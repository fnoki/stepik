...
def lose():
    if input("ты проиграл, еще раз? (y/n) ") == 'y':
        start_game()
    else:
        print('хорошо, пока')
...
def start_game():
    print("\nПривет, это виселица из детства, начнем")
    game()


def changer(value: str):
    global hidden_word
    global now
    global counter_false
    counter_false -= 1
    if value in hidden_word:
        count = hidden_word.count(value)
        while count != 0:
            equals = hidden_word.find(value)
            now = now[:equals] + value + now[equals + 1:]
            hidden_word = hidden_word[:equals] + '_' + hidden_word[equals + 1:]
            count -= 1
            print(display_hangman(counter_false))
    else:
        print(display_hangman(counter_false))
        if counter_false == 0:
            lose()
...
def game():
    global hidden_word
    global now
    hidden_word = pick_word()
    now = "_" * len(hidden_word)
    count_for_ans = 1
    print(f"Твое слово состоит из {len(hidden_word)} букв, у тебя будет всего 6 попыток!")
    print("_" * len(hidden_word))
    for_the_while = hidden_word
    while now != for_the_while:
        print(f'Какая будет твоя {count_for_ans} буква?')
        input_text = input()
        if len(input_text) == 1:
            print(changer(input_text)+"\n")
            count_for_ans += 1

        else:
            print('вводи по одной букве')
            continue
    else:
        print('Поздравляю, ты победил!!!')
        if input("сыграем, еще раз? (y/n) ") == 'y':
            start_game()
        else:
            print('хорошо, пока')
